...
| F11 | Info | **Verified clean:** payload schemas carry the actor fields the AUD mirror rule needs (`approved_by`/`rejected_by`/`executed_by`); all six `payout.*` permission keys bound in roles.yaml/registry; ledger postings (LED-07 at approve, LED-08 at `PayoutPaid`) match docs/05 §3 and the `reason_code` enum; the `payout-state.md` diagram == §3.3; audit tiers coexist coherently — the **event mirror** rows are sensitive per the docs/49 C2 rule (money-adjacent) while the direct approve/reject **action write** is critical per this module's spec (different records, no conflict). | No action. |
