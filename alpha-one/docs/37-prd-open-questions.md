...
Design-review questions (raised by the team, not the PRD workbook): **36** — **5 open** (§Design-review questions below).
...
| D32 | FUNDED accounts have no breach edge: the V1 machine routes FUNDED only to SUSPENDED/TERMINATED, so a breach verdict on a funded account has no automatic transition (docs/50 F13). Options to consider — A: FUNDED→BREACH_DETECTED reusing the breach path (terminates via CLOSING); B: FUNDED→SUSPENDED automatic + risk review decides; C: funded breach = force-terminate path with extra guard. Not decided in the tenth pass. | Tenth-pass review (docs/50 F13) | BE-1 + PAY owner | Phase 4 wave 3 (trading depth) or first funded-account incident | (open — raised 2026-09-19; owner decision pending) |
