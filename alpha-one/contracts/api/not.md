...
7. payout requested — **no V1 trigger (D37, docs/52)**: no `payout.requested` event exists in V1 (the 201 response + TD status is the trader ack; the queue reads the table); template reserved for the V2 event (docs/11 §4.2)
...
- Resolved 2026-09-19 (D37, docs/52): no `payout.requested` event in V1 (sheet lists none); template 7 has no V1 trigger. V2 may add the event + template (docs/11 §4.2).
